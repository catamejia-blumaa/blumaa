...
const CaseStudy = () => {
  ...
  if (!project) {
    return (
      <Layout>
        <div className="py-32 text-center container">
          <h1 className="text-4xl font-serif text-night mb-4">Case study not found</h1>
  ...
  return (
    <Layout>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            {project.website_url && (
              <a
                ...
                className="inline-flex items-center text-blue hover:text-night transition-colors text-sm gap-2"
              >
      ...
      <section className="pb-24 bg-white-chocolate">
        <div className="container max-w-4xl">
          <div className="mb-12">
            <h2 className="text-2xl font-serif text-night mb-4">About the Project</h2>
            <p className="text-night/80 leading-relaxed text-lg">{project.description}</p>
            <div className="flex flex-wrap gap-3 mt-6">
              {project.services.map((service, i) => (
                <span key={i} className="px-4 py-2 bg-blue/10 text-blue text-sm rounded-full">
                  {service}
                </span>
              ))}
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-16">
            {project.challenge && (
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <h2 className="text-2xl font-serif text-night mb-4">The Challenge</h2>
                <p className="text-night/80 leading-relaxed">{project.challenge}</p>
              </motion.div>
            )}
            {project.solution && (
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}>
                <h2 className="text-2xl font-serif text-night mb-4">The Solution</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {images.map((img) => (
                <div key={img.id} className="rounded-xl overflow-hidden border-2 border-blue/10">
                  ...
                  />
                  {img.caption && (
                    <p className="text-sm text-night/60 text-center py-2">{img.caption}</p>
                  )}
                </div>
              ))}
            </div>
      ...
      {(project.results || metrics.length > 0) && (
        <section className="py-24 bg-blue-dark text-white-chocolate">
          <div className="container max-w-4xl relative z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <h2 className="text-3xl md:text-4xl font-serif mb-8">The Results</h2>
              {project.results && (
                <p className="text-white-chocolate text-lg mb-8 leading-relaxed">{project.results}</p>
              )}
              {metrics.length > 0 && (
      ...
      <section className="py-24 bg-blue text-white-chocolate">
        <div className="container text-center relative z-10">
          <h2 className="text-4xl font-serif mb-6">Ready to be the next success story?</h2>
