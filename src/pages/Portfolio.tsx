      ...
      <Layout>
        <section className="py-24 md:py-32 bg-white-chocolate">
          <div className="container">
            <div className="text-center text-night/70">{pf.loading}</div>
          </div>
        </section>
      </Layout>
    );
      ...
      <section className="py-24 md:py-32 bg-white-chocolate">
        <div className="container">
          <motion.div initial="hidden" animate="visible" className="max-w-2xl mb-16">
            <motion.p variants={fadeUp} custom={0} className="text-blue font-semibold text-sm uppercase tracking-[0.2em] mb-4">{pf.badge}</motion.p>
            ...
            {projects.map((p, i) => (
              <motion.div key={p.slug} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp}>
                <Link to={`/portfolio/${p.slug}`} className="group block">
                  <div className="aspect-[4/3] bg-white rounded-xl mb-4 flex items-center justify-center overflow-hidden group-hover:shadow-lg transition-shadow border-2 border-blue/10 group-hover:border-blue relative">
                    {p.thumbnail_url ? (
                  ...
                  <h3 className="text-xl font-serif text-night group-hover:text-blue transition-colors mb-1">{p.title}</h3>
      ...
      <section className="py-24 bg-blue text-white-chocolate">
        <div className="container text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-4xl md:text-5xl font-serif mb-6 drop-shadow-sm">{pf.ctaH2}</h2>
            <p className="text-white-chocolate text-lg mb-8 max-w-xl mx-auto">{pf.ctaBody}</p>
            <Button asChild size="lg" className="bg-light-yellow text-night hover:bg-light-yellow/90 rounded-full px-10 font-semibold hover:scale-105 transition-all shadow-lg hover:shadow-xl">
              <Link to="/contact">{pf.ctaBtn} <ArrowRight className="ml-2" size={18} /></Link>
            </Button>
          </motion.div>
        </div>
      </section>
